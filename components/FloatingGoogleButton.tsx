...
const FloatingGoogleButton: React.FC = () => {
  ...
  return (
    <a
      ...
      aria-label={t('common.rateExperience')}
    >
      <div className="bg-premium-gold/20 p-1.5 rounded-full">
        <Star className="w-4 h-4 text-premium-gold fill-premium-gold" />
      </div>
      <span className="font-display font-bold text-xs tracking-wider uppercase hidden sm:inline">{t('common.rateExperience')}</span>
    </a>
  );
};
